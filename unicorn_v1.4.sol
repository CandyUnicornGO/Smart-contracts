...
contract BlackBoxController is BlackBoxAccessControl, usingOraclize  {
    ...
    //TODO gas limit
    function geneCore(uint childUnicornId, uint unicorn1_id, uint unicorn2_id) onlyBreeding public payable {
        if (oraclize_getPrice("URL") > this.balance) {
            revert();
        } else {
            string memory query = strConcat(geneCoreQuery1, uint2str(unicorn1_id),
    ...
    function geneCoreManual(uint _unicornId) onlyOwner public {
    ...
    */
    function clearApprovalAndTransfer(address _from, address _to, uint256 _unicornId) internal {
        ...
        if (marketAddress != address(0)) {
            UnicornMarketInterface(marketAddress).deleteOffer(_unicornId);
        }
    }
...
contract UnicornToken is UnicornBase {
    ...
    function UnicornToken(address _marketAddress, address _breedingAddress) public {
        marketAddress = _marketAddress;
        breedingAddress = _breedingAddress;
    }
...
contract UnicornBreedingAccessControl is UnicornAccessControl {
    ...
    // UnicornTokenInterface internal unicornTokenContract; //only on deploy
    ...
    function setUnicornToken(address _unicornTokenAddress) external onlyOwner whenPaused    {
        ...
        // unicornContract = UnicornTokenInterface(_unicornTokenAddress);
...
contract UnicornBreeding is UnicornBreedingAccessControl {
    ...
    function makeHybridization(uint _unicornId, uint _price) public returns (uint)    {
        UnicornTokenInterface unicorn = UnicornTokenInterface(unicornTokenAddress);
        require(unicorn.owns(msg.sender, _unicornId));
        require(unicorn.isUnfreezed(_unicornId));
    ...
    function acceptHybridization(uint _hybridizationId, uint _unicornId) whenNotPaused public payable {
        UnicornTokenInterface unicorn = UnicornTokenInterface(unicornTokenAddress);
        require(unicorn.owns(msg.sender, _unicornId));
        Hybridization storage h = hybridizations[_hybridizationId];
        require(h.exists && !h.accepted);
        require(_unicornId != h.unicorn_id);
        require(unicorn.isUnfreezed(_unicornId) && unicorn.isUnfreezed(h.unicorn_id));
        ...
        BlackBoxInterface(blackBoxAddress).geneCore.value(unicornManagement.oraclizeFee())(childUnicornId, h.unicorn_id, h.second_unicorn_id);

        unicorn.ownerOf(h.unicorn_id).transfer(h.price);
    ...
    function cancelHybridization (uint _hybridizationId) public     {
        ...
        require(h.exists && !h.accepted);
        UnicornTokenInterface unicorn = UnicornTokenInterface(unicornTokenAddress);
        require(unicorn.owns(msg.sender, h.unicorn_id));
    ...
    function createUnicorn() public payable whenNotPaused returns(uint256)   {
        ...
        return _createUnicorn(msg.sender, 0, 0);
    }
    ...
    function createPresaleUnicorn(address _owner) public payable onlyManager whenPaused returns(uint256) {
        ...
        return _createUnicorn(_owner == address(0) ? msg.sender : _owner, 0, 0);
    }
    ...
    function _createUnicorn(address _owner, uint _parent1Id, uint _parent2Id) private returns(uint256) {
        ...
        uint256 newUnicornId = UnicornTokenInterface(unicornTokenAddress).createUnicorn(_owner);
        BlackBoxInterface(blackBoxAddress).createGen0.value(unicornManagement.oraclizeFee())(newUnicornId);
    ...
    function plusFreezingTime(uint _unicornId) private {
        UnicornTokenInterface(unicornTokenAddress).plusFreezingTime(_unicornId);
    }

    function plusTourFreezingTime(uint _unicornId) onlyTournament public {
        UnicornTokenInterface(unicornTokenAddress).plusTourFreezingTime(_unicornId);
    }
    ...
    function minusFreezingTime(uint _unicornId) public {
        ...
        UnicornTokenInterface(unicornTokenAddress).minusFreezingTime(_unicornId, unicornManagement.subFreezingTime());
    }
    ...
    function minusTourFreezingTime(uint _unicornId) public {
        ...
        UnicornTokenInterface(unicornTokenAddress).minusTourFreezingTime(_unicornId, unicornManagement.subTourFreezingTime());
    }

    function getHybridizationPrice(uint _hybridizationId) public view returns (uint) {
        return unicornManagement.getHybridizationFullPrice(hybridizations[_hybridizationId].price);
    }
    ...
    function setGene(uint _unicornId, bytes _gene) onlyBlackBox public {
        UnicornTokenInterface(unicornTokenAddress).setGene(_unicornId, _gene);
    }
