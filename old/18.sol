...
contract UnicornBreeding is UnicornAccessControl {
    ...
    function acceptHybridization(uint _firstUnicornId, uint _secondUnicornId) whenNotPaused public payable {
        ...
        if (price > 0) {
            ...
            uint fee = fullPrice.sub(price);
            balances.tokenMinus(candyTokenAddress, msg.sender, fullPrice);
            balances.tokenPlus(candyTokenAddress, address(this), fee);
            balances.tokenPlus(candyTokenAddress, unicornToken.ownerOf(_firstUnicornId), price);
        }
    ...
    function selfHybridization(uint _firstUnicornId, uint _secondUnicornId) whenNotPaused public payable {
        ...
        if (selfHybridizationPrice > 0) {
            ...
            balances.tokenMinus(candyTokenAddress, msg.sender, selfHybridizationPrice);
            balances.tokenPlus(candyTokenAddress, address(this), selfHybridizationPrice);
        }
    ...
    function createUnicornForCandy() public payable whenNotPaused returns(uint256)   {
        ...
        balances.tokenMinus(candyTokenAddress, msg.sender, price);
        balances.tokenPlus(candyTokenAddress, address(this), price);
        return _createUnicorn(msg.sender);
    }
...
contract UnicornMarket is UnicornBreeding {
    ...
    function buyUnicornWithCandy(uint _unicornId) whenNotPaused public {
        ...
        if (price > 0) {
            ...
            balances.tokenMinus(candyTokenAddress, msg.sender, fullPrice);
            balances.tokenPlus(candyTokenAddress, address(this), fee);
            balances.tokenPlus(candyTokenAddress, owner, price);
        }
    ...
    */
    function tradeBalances(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        ...
        if (balances.isTrustedToken(tokenGet)) {
            ...
            TrustedTokenInterface(tokenGet).transferFromSystem(msg.sender, this, feeTakeXfer);
        } else {
            balances.tokenMinus(tokenGet, msg.sender, amount.add(feeTakeXfer));
            balances.tokenPlus(tokenGet, user, amount);
            balances.tokenPlus(tokenGet, address(this), feeTakeXfer);
        }
        ...
        if (balances.isTrustedToken(tokenGive)) {
            ...
            balances.tokenMinus(tokenGive, user, amountGive.mul(amount).div(amountGet));
            balances.tokenPlus(tokenGive, msg.sender, amountGive.mul(amount).div(amountGet));
        }
    }
...
contract UnicornContract is UnicornCoinMarket {
    ...
    function UnicornContract(address _breedingDB, address _balances, address _unicornManagementAddress) UnicornAccessControl(_unicornManagementAddress) public {
        candyTokenAddress = unicornManagement.candyToken();
        breedingDB = BreedingDataBaseInterface(_balances);
        balances = UnicornBalancesInterface();
    }
