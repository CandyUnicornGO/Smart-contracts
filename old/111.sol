...
contract UnicornBalances is UnicornAccessControl {
    ...
    bool depositingTokenFlag; // True when Token.transferFrom is being called from depositToken
...
*/
contract UnicornBreeding is UnicornBalances {
    ...
    function acceptHybridization(uint _firstUnicornId, uint _secondUnicornId) whenNotPaused public payable {
        ...
        if (price > 0) {
            ...
            require(tokens[address(candyToken)][msg.sender] >= fullPrice);
            tokens[address(candyToken)][msg.sender] = tokens[address(candyToken)][msg.sender].sub(fullPrice);
            tokens[address(candyToken)][address(this)] = tokens[address(candyToken)][address(this)].add(fullPrice);
        }
        ...
        if (price > 0) {
            tokens[address(candyToken)][address(this)] = tokens[address(candyToken)][address(this)].sub(fullPrice);
            tokens[address(candyToken)][unicornToken.ownerOf(_firstUnicornId)] = tokens[address(candyToken)][unicornToken.ownerOf(_firstUnicornId)].add(fullPrice);
        }
    ...
    function selfHybridization(uint _firstUnicornId, uint _secondUnicornId) whenNotPaused public payable {
        ...
        if (selfHybridizationPrice > 0) {
            require(tokens[address(candyToken)][msg.sender] >= selfHybridizationPrice);
            tokens[address(candyToken)][msg.sender] = tokens[address(candyToken)][msg.sender].sub(selfHybridizationPrice);
            tokens[address(candyToken)][address(this)] = tokens[address(candyToken)][address(this)].add(selfHybridizationPrice);
        }
    ...
    function createUnicornForCandy() public payable whenNotPaused returns(uint256)   {
        require(msg.value == unicornManagement.oraclizeFee());
        require(tokens[address(candyToken)][msg.sender] >= getCreateUnicornPriceInCandy());
        tokens[address(candyToken)][msg.sender] = tokens[address(candyToken)][msg.sender].sub(getCreateUnicornPriceInCandy());
        tokens[address(candyToken)][address(this)] = tokens[address(candyToken)][address(this)].add(getCreateUnicornPriceInCandy());
        return _createUnicorn(msg.sender);
    }
...
contract UnicornCoinMarket is UnicornMarket {
    ...
    */
    function tradeBalances(address tokenGet, uint amountGet, address tokenGive, uint amountGive, address user, uint amount) private {
        ...
        tokens[tokenGet][this] = tokens[tokenGet][this].add(feeTakeXfer);
...
contract UnicornContract is UnicornCoinMarket {
    function UnicornContract(address _breedingDB, address _unicornManagementAddress) UnicornAccessControl(_unicornManagementAddress) public {
